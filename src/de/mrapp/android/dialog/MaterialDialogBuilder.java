...
public class MaterialDialogBuilder extends AlertDialog.Builder {
	...
	/**
	 * The color of the list items of the dialog, which is created by the
	 * builder.
	 */
	private int itemColor = -1;

	/**
	 * The control color of the list items of the dialog, which is created by
	 * the builder.
	 */
	 ...
	 */
	private void showMessageTextView(final ViewGroup titleContainer, final TextView messageTextView) {
		messageTextView.setVisibility(View.VISIBLE);
		LinearLayout.LayoutParams layoutParams = (LayoutParams) titleContainer.getLayoutParams();
		layoutParams.bottomMargin = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_spacing);
	 ...
	 */
	private void inflateContentView(final View root, final ViewGroup titleContainer, final TextView messageTextView,
	...
	private void initializeListViewListener(final AlertDialog dialog, final ListView listView) {
		if (listViewChoiceMode == ListView.CHOICE_MODE_NONE) {
			listView.setOnItemClickListener(
					new OnItemClickListenerWrapper(listViewSingleChoiceListener, dialog, AlertDialog.BUTTON_POSITIVE));
		} else if (listViewChoiceMode == ListView.CHOICE_MODE_SINGLE) {
			listView.setOnItemClickListener(new OnItemClickListenerWrapper(listViewSingleChoiceListener, dialog, 0));
	 ...
	 *            an instance of the class {@link TextView}
	 */
	private void showContentContainer(final ViewGroup contentContainer, final ViewGroup titleContainer,
			final TextView messageTextView) {
		contentContainer.setVisibility(View.VISIBLE);
		int contentSpacing = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_spacing);
		LinearLayout.LayoutParams titleLayoutParams = (LayoutParams) titleContainer.getLayoutParams();
		titleLayoutParams.bottomMargin = contentSpacing;
		titleContainer.setLayoutParams(titleLayoutParams);
		LinearLayout.LayoutParams messageLayoutParams = (LayoutParams) messageTextView.getLayoutParams();
	 ...
	 *            {@link AlertDialog}
	 */
	private void initializeButtonBar(final View root, final ViewGroup buttonBarContainer, final AlertDialog dialog) {
	...
	private void showButtonBarContainer(final View root, final View buttonBarContainer) {
		...
		int paddingLeft = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_padding_left);
		int paddingTop = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_padding_top);
		int paddingRight = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_padding_right);
		int paddingBottom = context.getResources().getDimensionPixelSize(R.dimen.dialog_content_padding_bottom);
	...
	private Button addNegativeButton(final View root, final AlertDialog dialog) {
		if (!TextUtils.isEmpty(negativeButtonText)) {
			...
			OnClickListenerWrapper onClickListener = new OnClickListenerWrapper(negativeButtonListener, null, dialog,
	...
	private Button addNeutralButton(final View root, final AlertDialog dialog) {
		if (!TextUtils.isEmpty(neutralButtonText)) {
			...
			OnClickListenerWrapper onClickListener = new OnClickListenerWrapper(neutralButtonListener, null, dialog,
	...
	private Button addPositiveButton(final View root, final AlertDialog dialog) {
		if (!TextUtils.isEmpty(positiveButtonText)) {
			...
			OnClickListenerWrapper onClickListener = new OnClickListenerWrapper(positiveButtonListener, validators,
	...
	/**
	 * Sets the color of the message of the dialog, which is created by the
	 * builder.
	 * 
	 * @param color
	 *            The color, which should be set, as an {@link Integer} value
	...
	/**
	 * Sets the color of the list items of the dialog, which is created by the
	 * builder.
	 * 
	 * @param color
	 *            The color, which should be set, as an {@link Integer} value
	...
	/**
	 * Sets the control color of the list itemsof the dialog, which is created
	 * by the builder.
	 * 
	 * @param color
	 *            The color, which should be set, as an {@link Integer} value
	...
	@Override
	public final MaterialDialogBuilder setNegativeButton(final CharSequence text, final OnClickListener listener) {
	...
	@Override
	public final MaterialDialogBuilder setNegativeButton(final int resourceId, final OnClickListener listener) {
		return setNegativeButton(context.getText(resourceId), listener);
	}

	@Override
	public final MaterialDialogBuilder setPositiveButton(final CharSequence text, final OnClickListener listener) {
	...
	@Override
	public final MaterialDialogBuilder setPositiveButton(final int resourceId, final OnClickListener listener) {
		return setPositiveButton(context.getText(resourceId), listener);
	}

	@Override
	public final MaterialDialogBuilder setNeutralButton(final CharSequence text, final OnClickListener listener) {
	...
	@Override
	public final MaterialDialogBuilder setNeutralButton(final int resourceId, final OnClickListener listener) {
		return setNeutralButton(context.getText(resourceId), listener);
	}

	@Override
	public final MaterialDialogBuilder setItems(final CharSequence[] items, final OnClickListener listener) {
		listAdapter = new ArrayAdapter<CharSequence>(context, android.R.layout.simple_list_item_1, items);
	...
	@Override
	public final MaterialDialogBuilder setItems(final int resourceId, final OnClickListener listener) {
		return setItems(context.getResources().getTextArray(resourceId), listener);
	}

	@Override
	public final MaterialDialogBuilder setAdapter(final ListAdapter adapter, final OnClickListener listener) {
		listAdapter = adapter;
		listViewSingleChoiceListener = listener;
		listViewChoiceMode = ListView.CHOICE_MODE_NONE;
		return this;
	...
	@Override
	public final MaterialDialogBuilder setCursor(final Cursor cursor, final OnClickListener listener,
			final String labelColumn) {
		throw new UnsupportedOperationException("This method is not supported yet");
	}

	@Override
	public final MaterialDialogBuilder setSingleChoiceItems(final CharSequence[] items, final int checkedItem,
			final OnClickListener listener) {
		listAdapter = new ArrayAdapter<CharSequence>(context, android.R.layout.simple_list_item_single_choice, items);
		listViewSingleChoiceListener = listener;
	...
	@Override
	public final MaterialDialogBuilder setSingleChoiceItems(final int resourceId, final int checkedItem,
			final OnClickListener listener) {
		return setSingleChoiceItems(context.getResources().getTextArray(resourceId), checkedItem, listener);
	}

	@Override
	public final MaterialDialogBuilder setSingleChoiceItems(final ListAdapter adapter, final int checkedItem,
			final OnClickListener listener) {
		listAdapter = adapter;
		listViewSingleChoiceListener = listener;
		listViewChoiceMode = ListView.CHOICE_MODE_SINGLE;
	...
	@Override
	public final MaterialDialogBuilder setSingleChoiceItems(final Cursor cursor, final int checkedItem,
			final String labelColumn, final OnClickListener listener) {
		throw new UnsupportedOperationException("This method is not supported yet");
	}

	@Override
	public final MaterialDialogBuilder setMultiChoiceItems(final CharSequence[] items, final boolean[] checkedItems,
			final OnMultiChoiceClickListener listener) {
		listAdapter = new ArrayAdapter<CharSequence>(context, android.R.layout.simple_list_item_multiple_choice, items);
	...
	@Override
	public final MaterialDialogBuilder setMultiChoiceItems(final int resourceId, final boolean[] checkedItems,
			final OnMultiChoiceClickListener listener) {
		return setMultiChoiceItems(context.getResources().getTextArray(resourceId), checkedItems, listener);
	}

	@Override
	public final MaterialDialogBuilder setMultiChoiceItems(final Cursor cursor, final String isCheckedColumn,
			final String labelColumn, final OnMultiChoiceClickListener listener) {
		throw new UnsupportedOperationException("This method is not supported yet");
	}

	@Override
	public final MaterialDialogBuilder setOnItemSelectedListener(final OnItemSelectedListener listener) {
